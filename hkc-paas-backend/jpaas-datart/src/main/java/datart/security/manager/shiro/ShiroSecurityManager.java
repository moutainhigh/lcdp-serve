...
public class ShiroSecurityManager implements DatartSecurityManager {
    ...
    //TODO, 需要时再完善
    @Override
    public String loginJpaas(String tokenString) throws AuthException {
        logoutCurrent();
        try {
            tokenString = tokenString.replace("Bearer ", "");
            JPaasUser jPaasUser = null;
            JsonResult<JPaasUser> jsonResult = userClient.getLoginUserInfoByToken(tokenString);
            if (jsonResult != null && jsonResult.getData() != null) {
                jPaasUser = jsonResult.getData();
            }
            PasswordToken passwordToken = new PasswordToken(jPaasUser.getUsername(), jPaasUser.getPassword(), System.currentTimeMillis());
            login(passwordToken);
        }catch (Exception ex){
            Exceptions.e(ex);
        }
