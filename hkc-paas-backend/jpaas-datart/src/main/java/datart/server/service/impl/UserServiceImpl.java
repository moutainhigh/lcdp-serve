...
import datart.feign.UserClient;
...
public class UserServiceImpl extends BaseService implements UserService {
    ...
    public String login(PasswordToken passwordToken) {
        try {
            ...
            String jpaasToken = passwordToken.getJpaasToken();
            if (StringUtils.isNotEmpty(jpaasToken)) {
                JsonResult<JPaasUser> jsonResult = userClient.getLoginUserInfoByToken(jpaasToken);
                if(jsonResult != null && jsonResult.getData() != null){
                    JPaasUser  jPaasUser = (JPaasUser) jsonResult.getData();
                    String username = jPaasUser.getUsername();
                    // 查询用户是否存在
                    User user = getUserByName(username);
                    if (user == null) {
                        UserRegisterParam registerParam = new UserRegisterParam();
                        registerParam.setUserId(jPaasUser.getUserId());
                        registerParam.setUsername(jPaasUser.getUsername());
                        registerParam.setPassword(jPaasUser.getPassword());
                        registerParam.setEmail("admin@example.org");
                        // request.setGender("male".equals(jPaasUser.get) ? "男" : "女");
                        // 把jpaas用户数据插入
                        register(registerParam, false);

                        passwordToken = new PasswordToken(jPaasUser.getUsername(),
                                jPaasUser.getPassword(),
                                System.currentTimeMillis(), jpaasToken);
                    }
                }
                ...
                return Const.TOKEN_HEADER_PREFIX+jpaasToken;
            }
            securityManager.login(passwordToken);
