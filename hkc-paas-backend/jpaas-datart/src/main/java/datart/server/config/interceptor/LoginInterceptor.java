...
public class LoginInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        String loginType = request.getHeader(Const.LOGIN_TYPE);
        if (token != null && loginType.equals("")) {
            try {
        ...
        }else if(token != null && loginType.equals(Const.LOGIN_JPAAS) && !securityManager.isAuthenticated()){
            try {
                token = securityManager.loginJpaas(token);
                response.setHeader(Const.TOKEN, token);
                response.setHeader(Const.LOGIN_TYPE, loginType);
                return securityManager.isAuthenticated();
            } catch (Exception e) {
                loginException = e;
            }
