...
public class AppDataServiceImpl {
    ...
    //导出数据Key的前缀
    private  static  final String KEY_PREFIX="5.p.";
    //默认数据源名
    private  static  final  String DEFAULT_DS="portal";

    //应用相关表
    private static final String[] appTables = {
            "ins_msg_def@MSG_ID_@消息项目",
            "ins_msgbox_def@BOX_ID_@消息盒子",
            "ins_column_def@COL_ID_@栏目定义",
            "ins_msgbox_box_def@ID_@消息盒子关联项目"
     };
    ...
    public JsonResult install(String  data){
        ...
        if(logMap.size()>0){
            String detail="Portal库被覆盖的记录：" + JSONObject.toJSONString(logMap);
    ...
    public void delete( String appId){
        ...
        if(logMap.size()>0){
            String detail="Portal库删除的记录：" + JSONObject.toJSONString(logMap);
            LogContext.put(Audit.DETAIL, detail);
        }
    }
}
