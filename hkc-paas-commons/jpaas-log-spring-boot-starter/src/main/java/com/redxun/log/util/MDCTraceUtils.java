...
public class MDCTraceUtils {
    ...
    public static String getNextSpanId() {
        AtomicInteger spanNumberAtomic = spanNumber.get();
        int spanNumberInt = -1;
        if(spanNumberAtomic != null){
            spanNumberInt = spanNumber.get().incrementAndGet();
        }
        return StrUtil.format("{}.{}", getSpanId(), spanNumberInt);
    }
