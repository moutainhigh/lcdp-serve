...
<#noparse>
    ...
    <rx-layout class="rxLayout listPreview" style="padding: 10px">
        ...
        <div slot="center" style="">
            <rx-fit :isheader="isheader && formBoList.isShowSearch=='YES'" :ref="'rxfit_'+formBoList.key">
                                ...
                                <span class="search-btn-box">
                                    ...
                                    <#if (searchColumns?size > 0)>
                                       <#-- <span class="search-btn" @click.stop="searchShow" v-if="!isheader && formBoList.isShowSearch=='YES'">
                                        ...
                                        <rx-popover :comboBox="{'width':'700px'}" v-if="!isheader && formBoList.isShowSearch=='YES'">
                ...
                <#if formBoList.dataStyle=='list'>
                    <rx-grid :showPage="${(formBoList.isPage=='YES')?string('true','false')}"
                </#if>
                ...
                <rx-tree-table
                        ...
                        :showFooter="${(formBoList.showSummaryRow=='YES')?string('true','false')}"
                        :columns="columns"
                ...
                >
                    ...
                    <span slot="${cellEditor.field}" slot-scope="text">
                        ...
                        <#else>
                        <span :title="text" slot="<#if formBoList.useCondSql=='WEBREQ' || formBoList.useCondSql=='INTERFACE'>${cellEditor.field}<#else>${cellEditor.field?upper_case}</#if>"
                        ...
                        <#if (cellEditor.type=='control')>
                            ...
                            <#elseif (cellEditor.control=='datePicker')>
                                ...
                                <a-date-picker showTime format="${dateFormat}" @change="gridChangeDate($event,$event,{record:record,dataIndex:'${cellEditor.field}',format:'${dateFormat}',renderFormat:'${renderDateFormat}'})"  />
...
<#--                                    </#if>-->
                                ...
                                <#elseif (cellEditor.renderType=='STARTFLOW' && cellEditor.renderConf??)>
                                    <span v-if="canStartFlow(record,'${cellEditor.renderConf.status}')">
                                        <a @click='openStartFlowDialog(record,"${cellEditor.renderConf.alias}")' >启动</a>
                                    </span>
                                    <span v-else>
                                        已启动
                                    </span>
                    ...
                    <#if (formBoList.isExpandRow=='YES')>
                        <template :title="text" slot="expandedRowRender" slot-scope="{record,index}">
